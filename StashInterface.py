...
class stash_interface:
    ...
    def scrapePerformerFreeones(self, name):
        ...
        { name url twitter instagram birthdate ethnicity country eye_color height measurements fake_tits career_length tattoos piercings aliases details }
        }"""
        result = self.callGraphQL(query)
        try:
            if len(result['data']["scrapePerformerList"]) != 0:
                ...
                query ScrapePerformer($scraped_performer: ScrapedPerformerInput!){
                    ...
                    { url twitter instagram birthdate ethnicity country eye_color height measurements fake_tits career_length tattoos piercings aliases details }
                }"""
